...
def extract_repo_source(data_dir: str, repo_path: str) -> Iterable[dict]:
    ...
    for real_path, file_path in list_source_files(repo_dir):
        try:
            ...
            if enc is None:
                print(f"Could not guess encoding for file {real_path} ({enc_detect}), try utf-8.")
            enc = "utf-8"
            yield {
...
def step(state: AppState) -> AppState:
    ...
    if len(next_state.buffer) >= next_state.buffer_size:
        output_path = os.path.join(state.output_dir, f'block_{next_state.next_block_id:05d}.json.gz')
    ...
    # output at last repo
    if next_state.current_repo_id == len(next_state.repo_list) and len(next_state.buffer) > 0:
        output_path = os.path.join(state.output_dir, f'block_{next_state.next_block_id:05d}.json.gz')
