...
class GithubQueryScraper(object):
    def __init__(self, page_size: int, delta_hours: int, start_date: str, end_date: str,
                 block_size: int, output_dir: str, max_req_per_min: int = 10):
    ...
    def build_url(self, page_num: int):
        ...
        query_url = f'https://api.github.com/search/repositories?q=language:coq+created:{start_time}..{end_time}' \
    ...
    def get_page_items_with_retry(self, page_num: int, max_retry: int = 10):
        if max_retry <= 0:
            raise RuntimeError(f'fetching page {page_num} failed with maximal retry number reached')
    ...
    def scrape_current_period(self):
        ...
        for i in range(1, num_steps + 1):
            ...
            print(f'page {i} / {num_steps}, size {len(items)}, '
...
if __name__ == '__main__':
    ...
    parser.add_argument('--output_dir', type=str, default='/localdata/yichen/pri_2008-2010/0001')
    ...
    scraper = GithubQueryScraper(args.per_page, args.time_slot_length, args.start_date, args.end_date,
                                 args.block_size, args.output_dir, args.req_per_min)
    print(scraper)
    scraper.run_scraper()
